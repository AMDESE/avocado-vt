...
def cmd_status_output(
    cmd, shell=False, ignore_status=True, verbose=True, timeout=60, session=None
):
    """
...
def find_substring(string, pattern1, pattern2=None):
    ...
    if not ret:
        LOG.debug("Could not find matched string with pattern: %s", pattern)
        return None
    return ret[0]
...
def find_command(cmd):
    ...
    """
    LOG.warning(
        "Function utils_misc.find_command is deprecated. "
...
def kill_process_tree(pid, sig=signal.SIGKILL, send_sigcont=True, timeout=0):
    ...
    try:
        ...
        LOG.warning(
            "Trying to kill_process_tree with timeout but running"
...
def log_line(filename, line):
    ...
    """
    logging.warning(
        "Calling log functions from `utils_misc` is deprecated, "
...
def generate_random_string(length, ignore_str=string.punctuation, convert_str=""):
    """
...
def generate_tmp_file_name(file_name, ext=None, directory=data_dir.get_tmp_dir()):
    """
...
def archive_as_tarball(
    source_dir, dest_dir, tarball_name=None, compression="bz2", verbose=True
):
    """
    ...
    if mode_str not in tarfile.open.__doc__:
        raise exceptions.TestError(
            "compression %s is not supported method %s"
            % (mode_str, tarfile.open.__doc__)
        )

    tarball_name = get_archive_tarball_name(source_dir, tarball_name, compression)
...
def get_guest_service_status(session, service, service_former=None):
    ...
    status = "inactive"
    if (
        re.search(r"Loaded: loaded", output, re.M)
        and output.count("Active: active") > 0
    ):
        status = "active"
    elif re.search(r"running", output.lower(), re.M) and not re.search(
        r"not running", output.lower(), re.M
    ):
        status = "active"
...
def safe_rmdir(path, timeout=10, session=None):
    ...
    while int(time.time() - start_time) < timeout:
        ...
        try:
            ...
            raise exceptions.TestSetupFail(
                ...
                "%s from remote machine: %s " % (path, info)
            )
    ...
    if not success:
        raise OSError(
            ...
            "after %d s and %d attempts." % (path, timeout, attempts),
        )
...
def is_mounted(
    src, mount_point, fstype, perm=None, verbose=False, fstype_mtab=None, session=None
):
    """
...
def install_host_kernel(job, params):
    ...
    if install_type == "rpm":
        ...
        write_keyval(job.resultdir, {"software_version_kernel": k_basename})
        host_kernel.boot()
        ...
        k = utils_koji.KojiPkgSpec(
            tag=koji_tag, build=koji_build, package="kernel", subpackages=["kernel"]
        )
        ...
        LOG.info(
            "Installing kernel dependencies (-devel, -firmware) " "through %s",
            install_type,
        )
        ...
        write_keyval(
            job.resultdir,
            {"software_version_kernel": " ".join(c.get_pkg_rpm_file_names(k_deps))},
        )
        host_kernel.boot()
        ...
        write_keyval(job.resultdir, {"software_version_kernel": git_repo_version})
        host_kernel.boot()

    else:
        LOG.info("Chose %s, using the current kernel for the host", install_type)
        k_version = process.run("uname -r", ignore_status=True).stdout_text
        write_keyval(job.resultdir, {"software_version_kernel": k_version})
...
def install_disktest_on_vm(test, vm, src_dir, dst_dir):
    ...
    session.cmd("sync")
    session.cmd("cd %s; make;" % (os.path.join(disktest_dst, "src")))
    session.cmd("sync")
    session.close()
    ...
    while index < len(output):
        ...
        try:
            ...
            if index + tmp_index < len(output):
                raise ValueError(
                    "%s is not included in the known console "
...
def create_x509_dir(
    path, cacert_subj, server_subj, passphrase, secure=False, bits=3072, days=1095
):
    """
    ...
    if not secure:
        cmd_set.append(
            "%s rsa -in %s -out %sserver-key.pem" % (ssl_cmd, path + server_key, path)
        )
        ...
        for cpu in origin_str.strip().split(","):
            if "-" in cpu:
                ...
                try:
                    ...
                    LOG.error("Illegimate string in cpu " "information: %s" % cpu)
                    cpu_list = []
                    break
        cpu_list.sort()
        return cpu_list
...
class NumaInfo(object):
    ...
    def __init__(self, all_nodes_path=None, online_nodes_path=None, session=None):
        ...
        self.online_nodes_withcpumem = list(
            set(self.online_nodes_withcpu) & set(self.online_nodes_withmem)
        )
    ...
    def get_node_distance(self, node_id):
        ...
        status, output = cmd_status_output(cmd, shell=True, session=self.session)
        if status != 0:
    ...
    def get_all_node_meminfo(self):
        ...
        for node in self.get_online_nodes():
            node_meminfo = {}
            numa_sys = kernel_interface.SysFS(meminfo_file % node, session=self.session)
    ...
    def get_online_nodes_withmem(self):
        ...
        if check_isfile(online_nodes_mem, session=self.session):
            ...
            LOG.warning(
                "sys numa node with memory file not" "present, fallback to online nodes"
            )
    ...
    def get_online_nodes_withcpu(self):
        ...
        if check_isfile(online_nodes_cpu, session=self.session):
            ...
            LOG.warning(
                "sys numa node with cpu file not" "present, fallback to online nodes"
            )
            return self.online_nodes
        return cpu_str_to_list(nodes_info)
...
class NumaNode(object):
    ...
    def __init__(self, i=-1, all_nodes_path=None, online_nodes_path=None, session=None):
        """
        :param all_nodes_path: Alternative path to
                /sys/devices/system/node/possible. Useful for unittesting.
        :param online_nodes_path: Alternative path to
        ...
        if i < 0:
            host_numa_info = NumaInfo(
                all_nodes_path, online_nodes_path, session=self.session
            )
            available_nodes = list(host_numa_info.nodes.keys())
            self.cpus = self.get_node_cpus(available_nodes[-1]).split()
            if len(available_nodes) > 1:
                self.extra_cpus = self.get_node_cpus(available_nodes[-2]).split()
    ...
    def pin_cpu(self, pid, cpu=None, extra=False):
        ...
        if cpu:
            ...
            error_context.context("Pinning process %s to the available CPU" % pid)
...
def get_dev_major_minor(dev):
    ...
    try:
        ...
        raise exceptions.TestError(
            "Fail to get major and minor numbers of the "
            "device %s:\n%s" % (dev, details)
        )
...
def _get_backend_dir(params):
    ...
    """
    return os.path.join(data_dir.get_root_dir(), "backends", params.get("vm_type", ""))
...
def get_qemu_dst_binary(params):
    ...
    if check_isdir(library_path):
        library_path = os.path.abspath(library_path)
        qemu_dst_binary = "LD_LIBRARY_PATH=%s %s" % (library_path, qemu_binary_path)
    else:
...
def get_binary(binary_name, params):
    ...
    binary_path = get_path(
        _get_backend_dir(params), params.get(key_in_params, binary_name)
    )
    if not os.path.isfile(binary_path):
        LOG.debug(
            "Could not find params %s in %s, searching the " "host PATH for one to use",
            binary_name,
            binary_path,
        )
...
def get_qemu_version(params=None):
    ...
    version_raw = process.run(
        "%s -version" % qemu_binary, shell=True
    ...
    if None in list(version.values()):
        LOG.error(
            "Local install qemu version cannot be detected, "
...
def compare_qemu_version(major, minor, update, is_rhev=True, params={}):
    ...
    installed_version_value = (
        installed_version["major"] * 1000000
        + installed_version["minor"] * 1000
        + installed_version["update"]
    )
    compared_version_value = int(major) * 1000000 + int(minor) * 1000 + int(update)
...
def is_qemu_function_supported(params, ignore_error=False):
    ...
    unsupported_err_msg = params.get(
        "qemu_unsupported_err_msg",
        "This qemu version doesn't support " "this function.",
    )
...
class ForAllP(list):
    ...
    def __getattr__(self, name):
        def wrapper(*args, **kargs):
            ...
            for o in self:
                threads.append(
                    InterruptedThread(o.__getattribute__(name), args=args, kwargs=kargs)
                )
            for t in threads:
                t.start()
...
def verify_running_as_root():
    ...
    if os.getuid() != 0:
        raise exceptions.TestSkipError(
            ...
            "(currently running with user %s)" % getpass.getuser()
        )
...
def selinux_enforcing():
    ...
    """
    LOG.warning(
        "This function was deprecated, Please use " "utils_selinux.is_enforcing()."
    )
...
def get_winutils_vol(session, label="WIN_UTILS"):
    ...
    """
    return wait_for(
        lambda: get_win_disk_vol(session, condition="VolumeName='%s'" % label), 240
    )
...
def format_windows_disk(
    session,
    did,
    mountpoint=None,
    size=None,
    fstype="ntfs",
    labletype=utils_disk.PARTITION_TABLE_TYPE_MBR,
    force=False,
):
    """
    ...
    for disk in disks.splitlines():
        if re.search(r"DISK %s" % did, disk, re.I | re.M):
            ...
            partition_index = re.search(
                r"\*\s+Partition\s(\d+)\s+", session.cmd(cmds), re.M
            ).group(1)
...
def format_linux_disk(
    ...
    for line in disks:
        ...
        if did not in did_list:
            ...
            continue
        if not size:
            size_output = session.cmd_output_safe("lsblk -oKNAME,SIZE|grep %s" % kname)
...
def format_guest_disk(
    ...
    return format_linux_disk(
        session, did, all_disks_did, partition, mountpoint, size, fstype
    )
...
def get_linux_drive_path(session, did, timeout=120):
    ...
    if dev:
        return dev.groups()[0]
    LOG.error(
        "Can not get drive path by id '%s', " "command output:\n%s" % (did, output)
    )
    return ""
        ...
        if image_info:
            for line in image_info.splitlines():
                if line.find("file format") != -1:
                    ...
                    image_info_dict["dsize"] = int(
                        float(
                            normalize_data_size(dsize, order_magnitude="B", factor=1024)
                        )
                    )
...
def is_qemu_capability_supported(capability):
    ...
    if not check_isdir(qemu_path) or not len(os.listdir(qemu_path)):
        raise exceptions.TestError(
            "Missing the directory %s or no file " "exists in the directory" % qemu_path
        )
...
class KSMController(object):
    ...
    def is_ksm_running(self):
        ...
        if self.interface == "sysfs":
            running = process.run("cat %s" % self.ksm_params["run"]).stdout_text.strip()
        else:
    ...
    def set_ksm_feature(self, feature_args):
        ...
        if self.interface == "sysfs":
            ...
            for key, value in list(feature_args.items()):
                process.system(
                    "echo %s > %s" % (value, self.ksm_params[key]), shell=True
                )
        else:
...
def add_ker_cmd(kernel_cmdline, kernel_param, remove_similar=False):
    ...
    if " %s " % kernel_param in kernel_cmdline_cmp:
        ...
        kernel_cmdline = re.sub(
            " %s= " % kernel_param_key, " ", kernel_cmdline_cmp
        ).strip()
...
def rm_ker_cmd(kernel_cmdline, kernel_param):
    ...
    kernel_cmdline_cmp = " %s " % kernel_cmdline
    if " %s " % kernel_param in kernel_cmdline_cmp:
        kernel_cmdline = re.sub(" %s " % kernel_param, " ", kernel_cmdline_cmp).strip()
    return kernel_cmdline
...
def get_pci_devices_in_group(str_flag=""):
    ...
    """
    d_lines = process.run('lspci -bDnn | grep "%s"' % str_flag, shell=True).stdout_text
...
def get_bootloader_cfg(session=None):
    ...
    for path in bootloader_cfg:
        ...
        if not status:
            ...
            break
    if not cfg_path:
        LOG.error("Failed to locate bootloader config file " "in %s." % bootloader_cfg)
    return cfg_path
...
class VFIOController(object):
    ...
    def __init__(self, load_modules=True, allow_unsafe_interrupts=True):
        ...
        self.vfio_modules = {"vfio": [], "vfio_pci": [], "vfio_iommu_type1": []}
        ...
        for key, value in list(self.vfio_linux_modules.items()):
            if check_module(key, value):
                ...
                try:
                    ...
                    modules_error.append("Load module %s failed: %s" % (key, detail))
            else:
    ...
    def check_iommu(self):
        ...
        for line in grub_content.splitlines():
            if re.search("vmlinuz.*intel_iommu=on", line):
                return
        raise VFIOError(
            "Please add 'intel_iommu=on' to kernel and " "reboot to effect it."
        )
    ...
    def get_pci_iommu_group_id(self, pci_id, device_type=""):
        ...
        if len(pci_group_devices) == 0:
            raise exceptions.TestError(
                "Can't find device in provided pci group: %s" % pci_id
            )
        readlink_cmd = (
            "readlink /sys/bus/pci/devices/%s/iommu_group" % pci_group_devices[0]
        )
        try:
    ...
    def get_iommu_group_devices(self, group_id):
        ...
        """
        output = process.run(
            "ls /sys/kernel/iommu_groups/%s/devices/" % group_id
        ).stdout_text
        group_devices = []
...
class SELinuxBoolean(object):
    ...
    def __init__(self, params):
        ...
        if self.server_ip:
            ...
            try:
                ...
                self.rem_selinux_disabled = (
                    result.stdout_text.strip().lower() == "disabled"
                )
            except process.CmdError:
        ...
        self.remote_bool_var = params.get("remote_boolean_varible", self.local_bool_var)
        self.local_bool_value = params.get("local_boolean_value")
        self.remote_bool_value = params.get(
            "remote_boolean_value", self.local_bool_value
        )
    ...
    def get_sebool_remote(self):
        ...
        cmd = self.ssh_cmd + "'%s'" % (get_sebool_cmd + "'| awk -F'-->' '{print $2}''")
    ...
    def cleanup(self, keep_authorized_keys=False, auto_recover=False):
        ...
        if self.cleanup_local and not self.selinux_disabled:
            result = process.run(
                "setsebool %s %s" % (self.local_bool_var, self.local_boolean_orig)
            )
    ...
    def setup_local(self):
        ...
        result = process.run(
            "setsebool %s %s" % (self.local_bool_var, self.local_bool_value)
        )
        if result.exit_status:
...
class BgJob(object):
    def __init__(
        ...
        self.stdout_tee = get_stream_tee_file(
            stdout_tee, DEFAULT_STDOUT_LEVEL, prefix=STDOUT_PREFIX
        )
        self.stderr_tee = get_stream_tee_file(
            stderr_tee, stderr_level, prefix=STDERR_PREFIX
        )
...
class AsyncJob(BgJob):
    ...
    def output_prepare(self, stdout_file=None, stderr_file=None):
        raise NotImplementedError(
            "This object automatically prepares its own " "output"
        )

    def process_output(self, stdout=True, final_read=False):
        raise NotImplementedError(
            "This object has background threads "
...
def run_parallel(
    commands, timeout=None, ignore_status=False, stdout_tee=None, stderr_tee=None
):
    """
    ...
    for bg_job in bg_jobs:
        if not ignore_status and bg_job.result.exit_status:
            raise process.CmdError(
                command, bg_job.result, "Command returned non-zero exit status"
            )
...
def _wait_for_commands(bg_jobs, start_time, timeout):
    ...
    while not timeout or time_left > 0:
        ...
        read_ready, write_ready, _ = select.select(
            read_list, write_list, [], SELECT_TIMEOUT
        )
...
def get_sosreport(
    ...
    if not path:
        ...
        path = host_path = get_path(
            utils_logfile.get_log_file_dir(), "sosreport-%s" % report_name
        )
    if session:
    ...
    try:
        ...
        if session:
            LOG.info(
                "copying sosreport from remote host/guest path: %s " "to host path: %s",
                path,
                host_path,
            )
...
def verify_sev(session,params,vm):
    ...
    try:
        if os.path.exists(cvm_module_path):
            ...
            raise exceptions.TestCancel(f"Host support for {cvm_type} capability check failed.")
    except IOError as e:
    ...
    try:
        ...
        LOG.info(
                f"Guest cvm {cvm_type} capability check output: {output}")
    except Exception as e:
        raise exceptions.TestFail("Guest cvm {cvm_type} capability verify fail: %s" % str(e))
