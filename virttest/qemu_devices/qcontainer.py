    ...
    def machine_by_params(self, params=None):
        ...
        def pflash_handler(firmware_name, machine_params):
            ...
            if firmware_path and images:
                ...
                pflash_code_format = utils_misc.get_image_info(pflash_code_path)[
                    "format"
                ]
                ...
                pflash_vars_filename = params.get(firmware_name + "_vars_filename")
                if pflash_vars_filename:
                    pflash_vars_src_path = os.path.join(
        ...
        def machine_arm64_mmio(machine_params):
            ...
            devices.append(
                qdevices.QMachine(
                    params=machine_params, child_bus=bus, aobject="virtio-mmio-bus"
                )
            )
            return devices
    ...
    def images_define_by_variables(
        ...
        if fmt in ("ide", "ahci"):
            ...
            if Flags.FLOPPY_DEVICE in self.caps:
                devices[-1] = qdevices.QDevice(
                    "floppy", aobject=name, parent_bus={"busid": "drive_%s" % name}
                )
