...
def update_boot_option_ubuntu(args, grub_key=None, session=None, remove_args=None):
    ...
    cmd = "sed -i 's/%s=.*/%s=\"%s\"/g' %s" % (grub_key, grub_key, output, boot_cfg)
    if session:
        status, output = session.cmd_status_output(cmd)
    else:
        out = process.run(cmd, shell=True)
    ...
    if status:
        raise exceptions.TestError(
            "Failed to update boot option %s with %s: " "%s" % (grub_key, args, output)
        )
    # update grub
    cmd = "update-grub"
    if session:
    ...
    if status:
        raise exceptions.TestError("Failed to update grub to modify kernel " "cmdline")
...
def check_kernel_cmdline(session, remove_args="", args=""):
    ...
    try:
        ...
        raise exceptions.TestError(
            "Failed to get kernel commandline using %s:" " %s" % (proc_cmdline, info)
        )
...
def start_windows_service(session, service, timeout=120):
    ...
    while time.time() < end_time:
        ...
        if re.search(r"\bFAILED 1060\b", o, re.I):
            raise exceptions.TestError(
                "Could not start service '%s' " "(service not installed)" % service
            )
...
def get_windows_disk_drive(session, filename, extension="exe", tmout=240):
    ...
    """
    return get_windows_file_abs_path(session, filename, extension).split(":")[0]
    ...
    if re.findall("ntpdate|w32tm", time_command):
        ...
        if re.match("ntpdate", time_command):
            ...
            try:
                ...
                host_time = time.mktime(time.strptime(host_main, time_format)) + float(
                    "0.%s" % host_mantissa
                )
            except Exception:
        ...
        try:
            try:
                ...
                LOG.debug(
                    "(time_format, time_string): (%s, %s)", time_format, host_time_out
                )
                raise err
        finally:
        ...
        try:
            ...
            raise exceptions.TestError("The time string from guest is unexpected.")
        except Exception as err:
            LOG.debug(
                "(time_filter_re, time_string): (%s, %s)", time_filter_re, str_time
            )
            raise err
        ...
        try:
            try:
                ...
                LOG.debug("(time_format, time_string): (%s, %s)", time_format, str_time)
                raise err
        finally:
            locale.setlocale(locale.LC_TIME, loc)
    else:
        ...
        try:
            ...
            LOG.debug("(time_filter_re, time_string): (%s, %s)", time_filter_re, output)
            raise err
...
def get_memory_info(lvms):
    ...
    if not isinstance(lvms, list):
        raise exceptions.TestError("Invalid list passed to get_stat: %s " % lvms)
    ...
    try:
        ...
        raise exceptions.TestFail(
            "Could not fetch host free memory info, " "reason: %s" % e
        )
    ...
    for vm in lvms:
        ...
        if shm is None:
            raise exceptions.TestError(
                "Could not get shared meminfo from " "VM %s" % vm
            )
        meminfo += "%dM; " % shm
    meminfo = meminfo[0:-2] + "}"
...
def get_image_version(qemu_image):
    ...
    error_context.context(
        "Get qcow2 image('%s') version" % qemu_image.image_filename, LOG.info
    )
...
def update_qcow2_image_version(qemu_image, ver_from, ver_to):
    ...
    if ver_from == ver_to:
        return None
    error_context.context(
        "Update qcow2 image version from %s to %s" % (ver_from, ver_to), LOG.info
    )
...
class AvocadoGuest(object):
    def __init__(
        ...
        self.avocado_repo = self.params.get(
            "avocado_repo_path", "https://github.com/avocado-framework/avocado.git"
        )
        ...
        self.plugins_path = os.path.join(
            self.test_path, self.repo_name(self.avocado_repo), "optional_plugins"
        )
        self.prerequisites = {
            "packages": ["git", "tar"],
        ...
        if self.avocado_vt:
            ...
            if self.installtype == "pip":
                ...
                self.avocado_vt_repo = self.params.get(
                    "avocado_vt_repo",
                    "https://github.com/avocado-framework/avocado-vt.git",
                )
                self.avocado_vt_repo_branch = self.params.get(
                    "avocado_vt_repo_branch", ""
                )
        if not self.env_check():
            raise exceptions.TestError(
                "avocado env check failed, " "consult previous errors"
            )
        if not self.install_avocado():
            raise exceptions.TestError(
                "avocado installation failed, " "consult previous errors"
            )
    ...
    def env_check(self):
        ...
        if not utils_misc.make_dirs(self.test_path, session=self.session):
            ...
            return False
        if self.avocado_vt:
            cmd = "lsmod | grep %s || modprobe %s" % (self.kvm_module, self.kvm_module)
            if self.session.cmd_status(cmd, timeout=self.timeout) != 0:
        ...
        if "pip" in self.installtype:
            ...
            for plugin in self.plugins[self.installtype]:
                if self.pip_bin != "pip3":
                    ...
                    avocado_cmd = (
                        "%s; %s list | awk '/avocado-framework/ {print $2}' | tail -c 5"
                ...
                if status != 0:
                    LOG.error(
                        "Avocado plugin %s pip " "installation failed:\n%s",
                        plugin,
                        output,
                    )
                    return False
            ...
            if not self.git_install(self.avocado_repo, branch=self.avocado_repo_branch):
            ...
            for plugin in self.plugins[self.installtype]:
                ...
                if self.session.cmd_status(cmd, timeout=self.timeout) != 0:
                    ...
                    return False
            if self.avocado_vt and not self.git_install(
                self.avocado_vt_repo,
                make="requirements",
    ...
    def runtest(self):
        ...
        if self.avocado_vt:
            ...
            for test_each in self.testlist:
                ...
                testcase = os.path.join(
                    self.test_path, self.repo_name(self.test_repo), testcase
                )
        ...
        avocado_cmd += " --job-results-dir %s --job-timeout %d" % (
            self.result_path,
            self.timeout,
        )
        status, output = self.session.cmd_status_output(
        ...
        if status != 0:
            ...
            LOG.error(
                "Avocado cmd: %s has failures consult "
    ...
    def get_results(self):
        ...
        guest_results_dir = utils_misc.get_path(self.test.debugdir, self.vm.name)
        os.makedirs(guest_results_dir)
        LOG.debug("Guest avocado test results placed " "under %s", guest_results_dir)
        ...
        uncompress_cmd = "tar xjvf %s -C %s" % (results_tarball, guest_results_dir)
    ...
    def get_session(self):
        ...
        try:
            ...
            raise exceptions.TestError(
                "Unable to get VM session, " "skipped to run avocado test: %s" % detail
            )
...
def run_autotest(
    ...
    def copy_if_hash_differs(vm, local_path, remote_path):
        ...
        output = session.cmd_output(
            "md5sum %s" % remote_path, timeout=int(params.get("md5sum_timeout", 240))
        )
        if "such file" in output:
            ...
            LOG.warning("MD5 check for remote path %s did not return.", remote_path)
        ...
        if remote_hash == local_hash and directory_exists(destination_autotest_path):
            return None
        LOG.debug(
            "Copying %s to guest (remote hash: %s, local hash:%s)",
            basename,
            remote_hash,
            local_hash,
        )
    ...
    def extract(vm, remote_path, dest_dir):
        ...
        if (has_pbzip2 == 0) and "pbzip2" in pbzip2_path:
            ...
            options = "xvjmf"
        extract_cmd = "tar %s %s -C %s" % (options, basename, os.path.dirname(dest_dir))
        extract_cmd += " & wait ${!}"
        ...
        if autotest_dirname != os.path.basename(dest_dir):
            ...
            session.cmd("mv %s %s" % (autotest_dirname, os.path.basename(dest_dir)))
    ...
    def config_control(control_path, job_args=None):
        ...
        fd, temp_control_path = tempfile.mkstemp(
            prefix="control", dir=data_dir.get_tmp_dir()
        )
        os.close(fd)
    ...
    compressed_autotest_path = os.path.join(data_dir.get_tmp_dir(), "autotest.tar.bz2")
    ...
    kernel_install_path = os.path.join(autotest_path, "tests", "kernelinstall")
    ...
    autotest_tarball = copy_if_hash_differs(
        vm, compressed_autotest_path, compressed_autotest_path
    )
    if autotest_tarball:
        ...
        if not directory_exists(tests_dir):
            tarball_url = (
                "https://codeload.github.com/autotest/"
    ...
    destination_test_output_dir = os.path.join(destination_autotest_path, "results")
    config.set("COMMON", "test_output_dir", destination_test_output_dir)
    config.write(aux_file)
    aux_file.close()
    global_config_guest = os.path.join(destination_autotest_path, "global_config.ini")
    ...
    if not single_dir_install:
        vm.copy_files_to(
            autotest_local_path,
            os.path.join(destination_autotest_path, "autotest-local"),
        )
    ...
    vm.copy_files_to(
        temp_control_path, os.path.join(destination_autotest_path, "control")
    )
    ...
    if not kernel_install_present:
        kernel_install_dir = os.path.join(
            data_dir.get_root_dir(), "shared", "deps", "run_autotest", "kernel_install"
        )
        kernel_install_dest = os.path.join(
            destination_autotest_path, "tests", "kernelinstall"
        )
    ...
    if boottool_update:
        ...
        # the guest system
        boottool_path = os.path.join(
            data_dir.get_root_dir(), "shared", "deps", "run_autotest", "boottool.py"
        )
    ...
    # Run the test
    LOG.info(
        "Running autotest control file %s on guest, timeout %ss",
    ...
    if server_control_path:
        ...
        command = "python -x %s %s --verbose -t %s" % (
            autotest_local_path,
            server_control_path,
            job_tag,
        )
    ...
    try:
        ...
        try:
            ...
            if migrate_background:
                ...
                kwargs = {"cmd": cmd, "timeout": timeout, "print_func": LOG.info}
                bg = utils_misc.InterruptedThread(session.cmd_output, kwargs=kwargs)
                bg.start()

                while bg.is_alive():
                    LOG.info("Autotest job did not end, start a round of " "migration")
                ...
                if params.get("guest_autotest_verbosity", "yes") == "yes":
                    ...
                    verbose = ""
                session.cmd_output(
                    "python -x ./autotest-local %s control & wait ${!}" % verbose,
                    timeout=timeout,
                    print_func=LOG.info,
                )
        finally:
            ...
            if server_process:
                if server_process.is_alive():
                    utils_misc.kill_process_tree(
                        server_process.get_pid(), signal.SIGINT
                    )
                server_process.close()

                # Remove the result dir produced by server_process.
                job_tag = os.path.basename(server_control_path)
                server_result = os.path.join(autotest_path, "results", job_tag)
        ...
        if vm.is_alive():
            ...
            get_results_summary()
            raise exceptions.TestError(
                "Timeout elapsed while waiting " "for job to complete"
            )
        else:
            raise exceptions.TestError(
                "Autotest job on guest failed " "(VM terminated during job)"
            )
        ...
        if ignore_session_terminated:
            ...
            get_results(destination_autotest_path)
            raise exceptions.TestError(
                "Autotest job on guest failed " "(Remote session terminated during job)"
            )
    ...
    if not results:
        raise exceptions.TestFail(
            "Autotest control file run did not " "produce any recognizable results"
        )
    if bad_results:
        if len(bad_results) == 1:
            e_msg = "Test %s failed during control file execution" % bad_results[0]
        else:
            e_msg = "Tests %s failed during control file execution" % " ".join(
                bad_results
            )
...
def run_virt_sub_test(test, params, env, sub_type=None, tag=None):
    ...
    if sub_type is None:
        raise exceptions.TestError(
            "Unspecified sub test type. Please specify a" "sub test type"
        )
    ...
    for i in column_list:
        ...
        for j in row_list:
            ...
            if utils_misc.aton(check) or utils_misc.aton(check) == 0.0:
                ...
                average_list[column_list[i]][j] = "%.2f" % (
                    count / len(result_dict[column_list[i]][j])
                )

    return average_list


def get_driver_hardware_id(
    driver_path,
    mount_point=os.path.join(data_dir.get_tmp_dir(), "mnt-virtio"),
    storage_path=os.path.join(data_dir.get_tmp_dir(), "prewhql.iso"),
    re_hw_id="(PCI.{14,50})",
    run_cmd=True,
):
    """
    ...
    if not os.path.ismount(mount_point):
        process.system("mount %s %s -o loop" % (storage_path, mount_point), timeout=60)
...
class BackgroundTest(object):
    ...
    def __init__(self, func, params, kwargs={}):
        ...
        """
        self.thread = threading.Thread(target=self.launch, args=(func, params, kwargs))
        self.exception = None
...
def ntpdate(service_ip, session=None):
    ...
    try:
        ...
        raise exceptions.TestFail("Failed to set the date and time. %s" % detail)
...
def run_avocado_bg(vm, params, test, testlist=[], avocado_vt=False, ignore_status=True):
    """
...
class Stress(object):
    ...
    def __init__(
        ...
        self.stress_install_from_repo = (
            self.params.get("stress_install_from_repo") == "yes"
        )
        self.download_url = self.params.get(
            "download_url_%s" % stress_type, download_url
        )
        if not aurl.is_url(self.download_url) and not os.path.isabs(self.download_url):
            self.download_url = utils_misc.get_path(
                data_dir.DEPS_DIR, self.download_url
            )
        self.download_type = self.params.get(
            "download_type_%s" % stress_type, download_type
        )
        ...
        self.uninstall_cmds = self.params.get(
            "uninstall_cmds_%s" % stress_type, uninstall_cmds
        )
        ...
        self.dependency_packages = str(
            self.params.get(
                "%s_dependency_packages_list" % stress_type, dependency_packages
            )
        )
        ...
        self.downloaded_file_path = self.params.get(
    ...
    def app_running(self):
        ...
        if self.session:
            return (
                self.cmd_status(self.check_cmd, timeout=self.stress_shell_timeout) == 0
            )
        else:
            return (
                self.cmd_status(
    ...
    def download_stress(self):
        ...
        try:
            ...
            if not self.downloaded_file_path:
                raise exceptions.TestError(
                    "Tool is not downloaded or download"
                    " link for Tool is not provided"
                )
            file_type = process.getoutput("file %s" % self.downloaded_file_path)
            if "directory" not in file_type:
                self.base_name = archive.uncompress(self.downloaded_file_path, tmp_path)
            else:
        ...
        if self.remote_host:
            ...
            args = (
                self.remote_host.__getitem__("server_ip"),
                "scp",
                self.remote_host.__getitem__("server_user"),
                self.remote_host.__getitem__("server_pwd"),
                "22",
                source,
                self.dst_path,
            )
    ...
    def install(self):
        ...
        status, output = self.cmd_status_output(
            self.make_cmds, timeout=self.stress_shell_timeout
        )
        if status != 0:
            raise exceptions.TestError("Installation failed with output:\n %s" % output)
    ...
    def clean(self):
        ...
        if self.stress_type == "uperf":
            rm_cmd += " && rm -rf %s" % os.path.join(
                self.dst_path, os.path.basename(self.params.get("client_profile_uperf"))
            )
...
class HostStress(Stress):
    ...
    def __init__(
        ...
        dependency_packages='@Development Tools' for RHEL.
        """
        super(HostStress, self).__init__(
            stress_type,
            params,
            download_url=download_url,
            make_cmds=make_cmds,
...
def unload_stress(stress_type, params, vms=None, remote_server=False):
    ...
    if stress_type == "stress_in_vms":
        ...
        HostStress("stress", params, remote_server=remote_server).unload_stress()
...
class ServerClientStress(object):
    ...
    def __init__(self, params, env):
        ...
        if self.need_profile:
            ...
            if not self.profile.endswith(".xml"):
                raise exceptions.TestError("%s profile not valid", self.stress_type)
    ...
    def prepare_profile(self, fpath, pat_repl):
        ...
        try:
            with open(fpath, "r+") as profile_content:
                ...
                LOG.debug("In prepare profile: pattern and replacement : %s", pat_repl)
        ...
        for server_vm, client_vm in self.pair_vms:
            try:
                ...
                if self.need_profile:
                    ...
                    self.pat_repl.update({"serverip": str(server_vm.get_address())})
    ...
    def verify_unload_stress(self, params):
        ...
        for vm in self.vms:
            try:
                s_ping, o_ping = utils_net.ping(vm.get_address(), count=10, timeout=20)
                if s_ping != 0:
                    error = True
                    LOG.error("%s seem to have gone out of network", vm.name)
                else:
...
class RemoteDiskManager(object):
    ...
    def __init__(self, params):
        ...
        self.runner = remote_old.RemoteRunner(
            host=remote_host, username=remote_user, password=remote_pwd
        )
    ...
    def get_free_space(self, disk_type, path="/", vgname=None):
        ...
        if disk_type == "file":
            try:
                ...
                raise exceptions.TestError(
                    "Get %s space failed: %s." % (os.path.dirname(path), str(detail))
                )
        elif disk_type == "lvm":
            if re.search(vgname, output):
                try:
                    ...
                    raise exceptions.TestError(
                        "Get %s space failed: %s." % (vgname, str(detail))
                    )
            else:
                raise exceptions.TestError(
                    "Get %s space failed: %s." % (vgname, output)
                )

    def occupy_space(self, disk_type, need_size, path=None, vgname=None, timeout=60):
        """
        ...
        return self.create_image(
            disk_type, occupied_path, occupied_size, vgname, "occupied", False, timeout
        )
    ...
    def iscsi_login_setup(self, host, target_name, is_login=True):
        ...
        if is_login:
            ...
            if target_name not in output:
                raise exceptions.TestError(
                    "Discovery %s on %s failed." % (target_name, host)
                )
            ...
            if "successful" not in output:
                ...
                if device_name:
                    ...
                    raise exceptions.TestError(
                        "Can not find target '%s' after login." % target_name
                    )
        else:
            if target_name:
    ...
    def create_vg(self, vgname, device):
        ...
        try:
            ...
            LOG.error("Create vgroup '%s' on remote host failed:%s", vgname, detail)
            return False
    ...
    def create_image(
        ...
        if disk_type == "file":
            ...
            if sparse:
                cmd = "lvcreate -V %sG %s --name %s --size 1M" % (size, vgname, lvname)
            else:
        ...
        if result.exit_status:
            raise exceptions.TestFail("Create image '%s' on remote host failed." % path)
        else:
            return path
...
def check_dest_vm_network(
    vm, vm_ip, remote_host, username, password, shell_prompt=r"[\#\$]\s*$", timeout=60
):
    """
    Ping migrated vms on remote host.
    """
    runner = remote_old.RemoteRunner(
        host=remote_host, username=username, password=password, prompt=shell_prompt
    )
    ...
    if ping_failed:
        raise exceptions.TestFail(
            "Failed to ping %s: %s" % (vm.name, result.stdout_text)
        )
